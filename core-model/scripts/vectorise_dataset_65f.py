...
def vectorise_dataset(input_dir: Path, output_dir: Path,
    ...
    print(f"[INFO] Samples OK: {len(vecs)}   |  failed/skipped: {sum(fail_reasons.values())}")
    ...
    header = (["label"] +
            ...
            [f"dist_tip_{n}" for n in ["thumb", "index", "middle", "ring", "pinky"]])
    df = pd.DataFrame(
        np.column_stack([labels, X_scaled]),   # concat label + features
        columns=header
    )
